...
void showfuzzy(const auto& data, const auto& places){
  for(size_t i = 0; i<places.size(); ++i){
    auto end = places[i].start + places[i].length;
    for(size_t j = places[i].start; j < end; ++j){
...
int main (){
  ...
  auto places       = create_Net(inputLengths);
  ...
  for (const auto & A : result){
    for (const auto & a : A){
      ...
      outfile << std::accumulate(s.begin(), s.end(), std::to_string(s[0]), [](const std::string& acc, auto b){
